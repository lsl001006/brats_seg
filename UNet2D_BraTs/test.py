...
use_asyndgan_unet = True
if use_asyndgan_unet:
    from UNet_asyndgan import UNet # asyndgan Unet
else:
    from unet import Unet # original Unet
...
def main():
    ...
    if use_asyndgan_unet:
        ...
        model = Unet() # "1" is a meaningless arg
    

    model = model.cuda()

    # Data loading code
    img_paths = glob(r'/home/lsl/Research/brats_seg/dataset/testImage/*')
    mask_paths = glob(r'/home/lsl/Research/brats_seg/dataset/testMask/*')
    ...
    if val_args.mode == "GetPicture" or val_args.mode == "G":
        ...
        with warnings.catch_warnings():
            ...
            with torch.no_grad():
                val_pred_path = 'output/%s/'%args.name + '/Pred/'
                        ...
                        for idx in range(output.shape[2]):
                            for idy in range(output.shape[3]):
                                ...
                                if output[i,2,idx,idy] > 0.5:
                                    ...
                                    rgbPic[idx, idy, 2] = 0
                        imsave(val_pred_path + rgbName,rgbPic)

            torch.cuda.empty_cache()
        """
        将验证集中的GT numpy格式转换成图片格式并保存
        convert the *.npy files in validation/GT/ directory to png images 
        """
        ...
        for idx in tqdm(range(len(val_mask_paths))):
            ...
            for idx in range(npmask.shape[0]):
                for idy in range(npmask.shape[1]):
                    #坏疽(NET,non-enhancing tumor)(标签1, label 1) 红色 red
                    if npmask[idx, idy] == 1:
                        GtColor[idx, idy, 0] = 255
                        GtColor[idx, idy, 1] = 0
                        GtColor[idx, idy, 2] = 0
                    #浮肿区域(ED,peritumoral edema) (标签2, label 2) 绿色 green
                    elif npmask[idx, idy] == 2:
                        GtColor[idx, idy, 0] = 0
                        GtColor[idx, idy, 1] = 128
                        GtColor[idx, idy, 2] = 0
                    #增强肿瘤区域(ET,enhancing tumor)(标签4, label 4) 黄色 yellow
    ...
    if val_args.mode == "Calculate" or val_args.mode == "C":
        """
        计算各种指标:Dice、Sensitivity、PPV
        calculate metrics:Dice, Sensitivity, PPV
        """
        wt_dices = []
        tc_dices = []
        et_dices = []
        ...
        pbPath = glob("output/%s/" % args.name + "Pred/*.png")
        if len(maskPath) == 0:
            print("please generate png images first!")
            return
